...
#ifdef SFAX8_UART_CLKGATE_CTL
static void sfax8_uart_clk_gate(int on, struct uart_port *port)
{
	u32 val;

	val = get_module_clk_gate(SF_UART_SOFT_RESET, 0);
	if (on)
		val |= 0x11 << port->line;
	else
		val &= ~(0x11 << port->line);
	set_module_clk_gate(SF_UART_SOFT_RESET, val, 0);
}
#endif
...
static void sfax8_dma_probe_initcall(struct device *dev, struct uart_sf_port *uap)
{
	...
	if(of_find_property(dev->of_node, "use-dma", NULL))
