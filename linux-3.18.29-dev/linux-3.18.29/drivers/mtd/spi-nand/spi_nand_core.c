...
static int spinand_mtd_compat_read(struct mtd_info *mtd, loff_t from, size_t len, size_t *retlen,
	     u_char *buf)
{
	struct mtd_oob_ops ops = {
			.len = len,
	...
	ret_code = spinand_mtd_read(mtd, from, &ops);
	*retlen = ops.retlen;
...
static int spinand_mtd_compat_write(struct mtd_info *mtd, loff_t to, size_t len, size_t *retlen,
	      const u_char *buf)
{
	struct mtd_oob_ops ops = {
	    .len = len,
...
static int spinand_probe(struct spi_mem *mem)
{
	...
	ppdata.of_node = mem->spi->dev.of_node;
