...
{
	u8 erase_code = nor->erase_opcode;
...
static int spi_nor_erase(struct mtd_info *mtd, struct erase_info *instr)
{
	...
	/* "sector"-at-a-time erase */
		...
		while (len) {
			#ifdef CONFIG_MTD_SPI_NOR_USE_4K_SECTORS
			/*
			 * use SPINOR_OP_SE when there are enough regions.
			 */
			div_u64_rem(addr, mtd->sectorsize, &rem);

			if (!rem && (len >= mtd->sectorsize)) {
				if (spi_nor_erase_sector(nor, addr))
					goto erase_err;
	...
	size_t *retlen, const u_char *buf)
{
	...
	if (page_offset + len <= nor->page_size) {
		...
		for (i = page_size; i < len; i += page_size) {
			...
			ret = nor->write(nor, to + i, page_size,
					retlen, buf + i);
			if (ret)
				goto write_err;
		}
...
#ifdef CONFIG_MTD_SPI_NOR_USE_4K_SECTORS
	mtd->sectorsize = info->sector_size;
