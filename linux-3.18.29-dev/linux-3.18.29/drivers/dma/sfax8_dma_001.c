...
static struct debug_data *data_p;
#endif
...
{
	return single_open(file, debug_show, PDE_DATA(inode));
}
							...
							size_t count, loff_t *f_ops)
{
	...
	if (*f_ops > 0)
		return 0;
	printk("dma irq done info:\n");
	for (i = 0; i < data_p->done_cnt; i++){
		//tv = (struct timeval *)(data_p->data + i * sizeof(struct timeval));
		//printk("dma debug: index %d, %lu.%06lu\n", i, tv->tv_sec, tv->tv_usec);
		//time = (ktime_t *)(data_p->data + i * sizeof(ktime_t));
		cycle = (cycle_t *)(data_p->data + i * sizeof(cycle_t));
		printk("dma debug: index %d, %llu\n", i, *cycle);
	}
	printk("dma clear irq info:\n");
	for (i = 0; i < data_p->fill_cnt; i++){
		cycle = (cycle_t *)(data_p->data + (200 + i) * sizeof(cycle_t));
		printk("dma debug: index %d, %llu\n", i, *cycle);
	}
	printk("clear pcm rx fifo times : %llu\n", data_p->over_count);

	for (i = 0; i < (data_p->state_cnt > 1600 ? 1600 : data_p->state_cnt); i++) {
		cnt = (u8 *)(data_p->data + i * sizeof(u8) + 400 * sizeof(cycle_t));
		printk("dma debug fifo info: index %d, %d\n", i, *cnt & 0xff);
	}

	printk("clear pcm rx fifo cnt : %d\n", data_p->state);

	*f_ops += count;
	return 0;
}

static ssize_t debug_write(struct file *file, const char __user *buffer,
							size_t count, loff_t *f_ops)
{
	if (*f_ops > 0)
		return 0;
...
xfer_exit:
	...
	//ktime_t *time;
	cycle_t *cycle;
	//pch = container_of(&thrd, struct dma_pl330_chan, thread);
	pch = desc->pchan;
	if ((unsigned int)pch->fifo_addr == 0x18400018  && !ret) {
		//memcopy(data_p->data + (data_p->fill_cnt + 200) * sizeof(struct timeval), &time, sizeof(struct timeval));
		//time = (struct timeval *)(data_p->data + (data_p->fill_cnt + 200) * sizeof(struct timeval));
		//do_gettimeofday(time);
		cycle = (cycle_t *)(data_p->data + (data_p->fill_cnt + 200) * sizeof(cycle_t));
		*cycle = gic_read_count();
		data_p->fill_cnt++;
		data_p->fill_cnt = data_p->fill_cnt % 200;
	}
...
#ifdef DEBUG
static void clear_pcm_overflow(void )
{
	void __iomem *regs = (void __iomem *)0xb8400000;
	u32 tmp, nu;
	u8 *data;
	u32 val = readl(regs + 0x20);
	nu = ((readl(regs + 0x24) >> 4) & 0x3f);

	if ( (val & 0x1) || (nu >= 30)) {
		data_p->over_count++;
		//clear pcm overflow interrupts
		data_p->state = nu;
		writel(0xffffffff, regs + 0x20);
		...
		do {
			udelay(10);
		} while (!(readl(regs + 0x24) & 0x8));

		printk("rx fifo state is 0x%x\n", readl(regs + 0x24));

		//step 4. enable pcm clk
		tmp = readl(regs + 0x8);
		tmp |= 0xF;
		writel(tmp, regs + 0x8);
	}
	...
	if (nu > 20) {
		data = (u8 *)(data_p->data + (data_p->state_cnt % 1600) * sizeof(u8) + 400 * sizeof(cycle_t));
		*data = (u8)nu;
		data_p->state_cnt++;
	}
}
...
static int pl330_update(struct pl330_dmac *pl330)
{
	...
	for (ev = 0; ev < pl330->pcfg.num_events; ev++) {
		if (val & (1 << ev)) { /* Event occurred */
			...
			#ifdef DEBUG
			pch = descdone->pchan;
			if ((unsigned int)pch->fifo_addr == 0x18400018) {
				clear_pcm_overflow();
			}
			#endif
			...
			//pch = container_of(&thrd, struct dma_pl330_chan, thread);
			if ((unsigned int)pch->fifo_addr == 0x18400018) {
				//memcopy(data_p->data + data_p->done_cnt * sizeof(struct timeval), &time, sizeof(struct timeval));
				//time = (struct timeval *)(data_p->data + data_p->done_cnt * sizeof(struct timeval));
				//do_gettimeofday(time);
				//time = (ktime_t *)(data_p->data + data_p->done_cnt * sizeof(ktime_t));
				//*time = ktime_get();
				cycle = (cycle_t *)(data_p->data + data_p->done_cnt * sizeof(cycle_t));
				*cycle = gic_read_count();
				data_p->done_cnt++;
				data_p->done_cnt = data_p->done_cnt % 200;

				cycle = (cycle_t *)(data_p->data + (data_p->fill_cnt + 200 )* sizeof(cycle_t));
				*cycle = cycle_tmp;
				data_p->fill_cnt++;
				data_p->fill_cnt = data_p->done_cnt % 200;
			}
			#endif
			#endif
...
static struct dma_pl330_desc *pl330_get_desc(struct dma_pl330_chan *pch)
{
	...
	spin_lock_irqsave(&get_pool_lock, flags);
	...
	spin_unlock_irqrestore(&get_pool_lock, flags);
		...
		unsigned long flags)
{
	...
	//if ((unsigned int)pch->fifo_addr == 0x18400018) state_cnt = 0;
	if (((unsigned int)pch->fifo_addr == 0x18400018)) {
		data_p->done_cnt = 0;
		data_p->fill_cnt = 0;
		memset(data_p->data, 0, sizeof(cycle_t) * 400);
	}
...
pl330_probe(struct platform_device *adev)
{
	...
	pl330->bus_clk = devm_clk_get(&adev->dev, NULL);

	if (IS_ERR(pl330->bus_clk)) {
		printk("gdma bus clock not available\n");
	} else {
		ret = clk_prepare_enable(pl330->bus_clk);
		if (ret) {
			dev_err(&adev->dev, "failed to enable gdma bus clock\n");
			return PTR_ERR(pl330->bus_clk);
		}
	}
	...
	file = proc_create_data("dma_debug", 0644, NULL, &debug_ops, NULL);
	if (!file)
		dev_err(&adev->dev, "can not creat debug proc file\n");
	data_p = (struct debug_data *)devm_kzalloc(&adev->dev, sizeof(struct debug_data), GFP_KERNEL);
	if (!data_p)
		dev_err(&adev->dev, "can not malloc memory for debug dataa\n");
	data_p->data = (u8 *)devm_kzalloc(&adev->dev, sizeof(cycle_t) * 600, GFP_KERNEL);
	if (!data_p->data)
		dev_err(&adev->dev, "can not malloc memory for debug data\n");
	data_p->done_cnt = 0;
	data_p->fill_cnt = 0;
	data_p->state_cnt = 0;
	data_p->over_count = 0;
...
probe_err2:
	if(!IS_ERR(pl330->bus_clk))
		clk_disable_unprepare(pl330->bus_clk);
	pl330_del(pl330);
