...
static inline int _ldst_peripheral(unsigned dry_run, u8 buf[],
				 const struct _xfer_spec *pxs, int cyc,
				 enum pl330_cond cond)
{
	int off = 0;
		...
		const struct _xfer_spec *pxs, int cyc)
{
	...
	case DMA_MEM_TO_DEV:
		/* fall through */
	case DMA_DEV_TO_MEM:
		off += _ldst_peripheral(dry_run, &buf[off], pxs, cyc,
			cond);
		break;
 ...
 * for mem-to-mem.
 */
static int _dregs(unsigned int dry_run, u8 buf[],
		const struct _xfer_spec *pxs, int transfer_length)
{
	...
	case DMA_MEM_TO_DEV:
		/* fall through */
	case DMA_DEV_TO_MEM:
		off += _ldst_peripheral(dry_run, &buf[off], pxs,
...
static int pl330_update(struct pl330_dmac *pl330)
{
	...
	for (ev = 0; ev < pl330->pcfg.num_events; ev++) {
		if (val & (1 << ev)) { /* Event occurred */
			...
			pch = descdone->pchan;
			chan = &pch->chan;
			if (chan->device->device_clear_exception &&
					chan->device->device_exception_data)
				chan->device->device_clear_exception(chan->device->device_exception_data);
...
pl330_probe(struct platform_device *adev)
{
...
#if 1
	{
		...
		cpu = CONFIG_NR_CPUS - 3;
		cpumask_set_cpu(cpu, &mask);
		for (i = 0; i < SFDMA_NR_IRQS; i++) {
			if (irq_set_affinity_hint(irq + i, &mask)) {
				dev_err(&adev->dev, "can not set irq %d affinity!\n", irq + i);
			}
