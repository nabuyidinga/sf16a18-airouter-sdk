...
{
	struct spi_transfer *xfer;
	bool keep_cs = false, need_reset = false;
	int ret = 0;
	unsigned long long ms = 1;
	int tries = 5;

retry:
	if (need_reset && master->reset_hardware)
		master->reset_hardware(master, msg->spi);
 ...
 */
void spi_flush_queue(struct spi_master *ctlr)
{
