...
void snd_dmaengine_pcm_register_clean_execption(struct device *dev,
			int stream, void (*clean)(void *data))
{
	struct snd_soc_platform *platform;
	struct dmaengine_pcm *pcm;
	struct dma_chan *chan;
	...
	chan->device->device_clear_exception = clean;
	chan->device->device_exception_data = dev;
}
EXPORT_SYMBOL_GPL(snd_dmaengine_pcm_register_clean_execption);

void snd_dmaengine_pcm_unregister_clean_execption(struct device *dev, int stream)
{
	struct snd_soc_platform *platform;
	struct dmaengine_pcm *pcm;
	struct dma_chan *chan;
	...
	pcm = soc_platform_to_pcm(platform);
	chan = pcm->chan[stream];
	chan->device->device_clear_exception = NULL;
	chan->device->device_exception_data = NULL;
}
EXPORT_SYMBOL_GPL(snd_dmaengine_pcm_unregister_clean_execption);
MODULE_LICENSE("GPL");
