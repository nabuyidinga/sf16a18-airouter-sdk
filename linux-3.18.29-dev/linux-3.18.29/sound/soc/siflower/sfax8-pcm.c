...
static long long clear_state = 0;
#endif
							...
							size_t count, loff_t *f_ops)
{
	...
	printk("clear pcm rx fifo cnt : %lld\n", clear_state);
...
static void sf_pcm_clear_fifo(void *data)
{
	struct sf_pcm_info *pcm;
	void __iomem *regs;
	u32 tmp, nu, val, timeout;
	struct device *dev = (struct device *)data;
	pcm = (struct sf_pcm_info *)dev_get_drvdata(dev);
	if (!pcm) {
		dev_err(dev, "get drv data failed\n");
		return ;
	}
	regs = pcm->regs;
	val = readl(regs + SF_PCM_IRQSTAT);
	nu = ((readl(regs + SF_PCM_FIFOSTAT) >> 4) & 0x3f);
	timeout = 10;

	if ( (val & 0x1) || (nu >= 28)) {
		clear_state++;
		//clear pcm overflow interrupts
		writel(0xffffffff, regs + SF_PCM_IRQSTAT);

		//step 1. stop pcm clk
		tmp = readl(regs + SF_PCM_CLKCTL);
		tmp = tmp & 0xFFFFFFF0;
		writel(tmp, regs + SF_PCM_CLKCTL);
		//step 2. disable rx
		tmp = readl(regs + SF_PCM_MC);
		tmp &= ~(1 << 4);
		tmp &= ~(1 << 3);
		tmp &= ~(1 << 0);
		writel(tmp, regs + SF_PCM_MC);
		//step 3. stop rx dma
		tmp = readl(regs + SF_PCM_CTL);
		tmp &= ~(1 << 2);
		writel(tmp, regs + SF_PCM_CTL);
		//step 4. enable rx 
		tmp = readl(regs + SF_PCM_MC);
		tmp |= (1 <<  4);
		tmp |= (1 <<  0);
		...
		//step 5. enable rx dma
		tmp = readl(regs + SF_PCM_CTL);
		tmp |= (1 << 2);
		writel(tmp, regs + SF_PCM_CTL);

		while (!(readl(regs + SF_PCM_FIFOSTAT) & 0x8) && timeout--)
			udelay(5);

		//step 6. enable pcm clk
		tmp = readl(regs + SF_PCM_CLKCTL);
		tmp |= 0xF;
		writel(tmp, regs + SF_PCM_CLKCTL);
	}
...
*/
	...
	snd_dmaengine_pcm_register_clean_execption(&pdev->dev,
			SNDRV_PCM_STREAM_CAPTURE, sf_pcm_clear_fifo);
#ifdef DEBUG
	{
	struct proc_dir_entry *file;
	file = proc_create_data("pcm_debug", 0644, NULL, &debug_ops, NULL);
	if (!file)
		dev_err(&pdev->dev, "can not creat debug proc file\n");
	}
#endif
...
static int sf_pcm_dev_remove(struct platform_device *pdev)
{
	...
	mem_res = platform_get_resource(pdev, IORESOURCE_MEM, 0);
	release_mem_region(mem_res->start, resource_size(mem_res));
	snd_dmaengine_pcm_unregister_clean_execption(&pdev->dev, SNDRV_PCM_STREAM_CAPTURE);
