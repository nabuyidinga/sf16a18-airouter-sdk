...
static int es7210_read(u8 reg, u8 *rt_value, struct i2c_client *client)
{
	...
	printk("reg is 0x%x; addr is 0x%x\n", reg, client->addr);
...
static void es7210_set_channel(int channel)
{
	/*
...
{
	printk("enter into %s\n", __func__);
	es7210_multi_chips_update_bits(ES7210_ADC34_MUTE_REG14, 0x03, 0x01);
...
{
	u8 tmp = 0;
	tmp = ucontrol->value.integer.value[0] << 1;
...
static int __init es7210_modinit(void)
{
	...
	printk("%s enter es7210 %d\n",__func__, ES7210_I2C_BUS_NUM);
