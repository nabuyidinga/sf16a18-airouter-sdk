...
static int es7149_trigger(struct snd_pcm_substream *substream,
		int cmd, struct snd_soc_dai *dai)
{
	struct device *dev;
	struct es7149_gpios *gpios;
	dev = dai->dev;
	gpios = (struct es7149_gpios *)dev_get_drvdata(dev);
	if (!gpios)
		dev_err(dev, "can not get drv data!\n");
	//printk("%s cmd is %d\n", __func__, cmd);
	switch (cmd) {
	case SNDRV_PCM_TRIGGER_STOP:
	case SNDRV_PCM_TRIGGER_SUSPEND:
	case SNDRV_PCM_TRIGGER_PAUSE_PUSH:
		gpio_set_value(gpios->speaker_gpio, 0);
		break;
	...
	case SNDRV_PCM_TRIGGER_PAUSE_RELEASE:
		...
		gpio_set_value(gpios->speaker_gpio, 1);
		udelay(100);
		break;
	}
...
static struct snd_soc_dai_ops es7149_ops = {
	.trigger = es7149_trigger,
};
...
static int es7149_gpio_init(struct platform_device *pdev)
{
	...
	ret = gpio_direction_output(gpios->speaker_gpio, 0);
	if (ret)
