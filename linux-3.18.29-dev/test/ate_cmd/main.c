...
#define SFCFG_CMD_ATE_SET_FREQ_OFFSET	0x0108
...
int32_t set_rf_freq_offset(char *tmpdata)
{
	struct iwreq wrq;
	struct sfcfghdr tmp;
	memset(&wrq, 0, sizeof(struct iwreq));
	...
	// cmd ate band
	tmp.magic_no = SFCFG_MAGIC_NO;
	tmp.comand_type = SFCFG_CMD_ATE_SET_FREQ_OFFSET;;
	tmp.comand_id   = SFCFG_CMD_ATE_SET_FREQ_OFFSET;;
	tmp.length      = 20;
	tmp.sequence    = 1;
	...
	if(do_ioctl(SFCFG_PRIV_IOCTL_ATE, &wrq) < 0){
		printf("SFCFG_PRIV_IOCTL_ATE ioctl SFCFG_CMD_ATE_SET_FREQ_OFFSET failed\n");
		return -1;
	}
	return 0;
...
int32_t set_pkg_fc(char *tmpdata)
{
	...
	wrq.u.data.pointer = (caddr_t)&tmp;
	wrq.u.data.length = sizeof(struct sfcfghdr) - sizeof(((struct sfcfghdr *)0)->data) + tmp.length;
	printf("length(%d)=sizeof(struct sfcfghdr)=%d - sizeof(((struct sfcfghdr *)0)->data)=%d + tmp.length=%d\n",
			wrq.u.data.length, sizeof(struct sfcfghdr), sizeof(((struct sfcfghdr *)0)->data), tmp.length);
...
int save_XO_config_to_file(char *value){
	uint16_t val = 0;
	FILE *fp = fopen("/lib/firmware/XO_config.txt","w+");
	if(fp == NULL){
	...
	if(system("dd if=/lib/firmware/XO_config.txt of=/dev/mtdblock3 bs=1 count=4 seek=32") == -1)
		return -1;
	return 0;
}
...
static void usage()
{
	...
	//printf(" ate_cmd wlan0 set ATETXXOF = XXXX, Set Freq offset\n");
	...
	printf(" ate_cmd wlan0 set ATE = ATEMACHDR -f 0x80 -t 0x00 -b 00:aa:bb:cc:dd:ee -d 16:88:aa:bb:cc:dd -s 16:88:aa:bb:cc:dd -q 0x5e20, Set the frame mac header\n");
	printf(" ate_cmd wlan1 fastconfig -n 10 -l 1024 -f 5500 -c 5500 -w 3 -u 3 -m 4 -i 9 -g 0 -p 28 -t, Transmit frames by fast setting on HB\n");
	...
	printf(" if you use (ate_init_lb/hb or ate_init lb/hb) to active ate tool, only use wlan0\n");
...
int32_t main(int32_t argc, char *argv[])
{
	...
	if(strcmp(argv[1],"save") == 0){
		char value[] = "0";
		strcpy(value, argv[2]);
		if(save_XO_config_to_file(value))
			return -3;
		goto DONE;
	}
	...
	{
		int opt;
		char options[] = "trskyqoxn:l:f:c:w:u:m:i:g:p:j:v:z:a:b";
	...
	if(argc > 5 && strcmp(argv[0],"ate_cmd") == 0 && strcmp(argv[4],"=") == 0)
	{
		...
		if(strcmp(argv[3],"ATETXXOF") == 0)
			set_rf_freq_offset(tmpdata);
