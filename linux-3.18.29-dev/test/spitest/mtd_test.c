...
int  mtd_test(char* arg, int fd_in){
	...
	for(loops = 0; loops < 100; loops++) {
		...
		gettimeofday(&t2,NULL);
		int time1 = t1.tv_sec*1000*1000+t1.tv_usec;
		int time2 = t2.tv_sec*1000*1000+t2.tv_usec;
		printf("-----%d usec\n",time2 - time1);
#if 0
...
#endif
		if(do_read_write_test(fd,tx,rx,1) < 0 ){
		...
		err = pthread_create(&(tid[1]), NULL, &do_speed_test, (void *)&data);
		if (err != 0)
			printf("\ncan't create thread :[%s]", strerror(err));
		else
			printf("\n Thread %d created successfully\n", (int)tid[1]);
		...
		while(1){
			...
			int kill_1 = pthread_kill(tid[0],0);
