...
#$1 p10 p10m p20 86v air001 clean default p10m
if [ -n "$1" ]; then
	board=$1
fi
...
case ${board} in
	...
	p20)
		...
		;;
	86v)
		target_board=target/linux/siflower/sf16a18_p10_${chip}_86v.config
		;;
	air001v1)
		target_board=target/linux/siflower/sf16a18_air001_v1_${chip}_def.config
		;;
	air001v2)
		target_board=target/linux/siflower/sf16a18_air001_v2_${chip}_def.config
		;;
	air001v4)
		target_board=target/linux/siflower/sf16a18_air001_v4_${chip}_def.config
		;;
	air001v5)
		target_board=target/linux/siflower/sf16a18_air001_v5_${chip}_def.config
		;;
	clean)
...
case ${imgtype} in
	...
	flash)
		if [ -d  package/siflower/luci-siflower ]; then
			mv package/siflower/flash/luci-siflower-flash package/siflower/
			mv package/siflower/luci-siflower package/siflower/flash/
		fi
		;;
	rel)
		if [ -d  package/siflower/luci-siflower-flash ]; then
			mv package/siflower/luci-siflower-flash package/siflower/flash/
			mv package/siflower/flash/luci-siflower package/siflower/
		fi
		;;
	*)
