...
detect_mac80211() {
	...
	for _dev in /sys/class/ieee80211/*; do
		...
		iw phy "$dev" info | grep -q '2412 MHz' || { mode_band="a"; channel="36"; ssidprefix="5G-"; htmode="HT40";}
		...
		if [ "$macprefix" = "" ]; then
			macprefix=`cat /sys/class/ieee80211/${dev}/macaddress | cut -c 13- | sed 's/://g'`
		fi
		ssid=irouter-$macprefix
