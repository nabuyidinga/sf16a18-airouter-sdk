...
--					/* modified by ivan 2015.7.31 remove "WAN6" under "Network - Interfaces" */
					if ifc ~= "loopback" and ifc ~= "wan6" then
...
--			/* start modified by ivan 2015.7.31 remove "Hostnames" under "Network" */
--			page = node("admin", "network", "hosts")
--			page.target = cbi("admin_network/hosts")
--			page.title  = _("Hostnames")
--			page.order  = 40
--			/* end modified by ivan 2015.7.31 remove "Hostnames" under "Network" */
		end

--		/* start modified by ivan 2015.7.31 remove "Static Routes" and "Diagnostics" under "Network" */
--[[
		...
		page.order  = 60
--]]
--		/* end modified by ivan 2015.7.31 remove "Static Routes" and "Diagnostics" under "Network" */
...
function wifi_delete(network)
	...
	if wnet then
		...
		if dev then
            ...
            --sysutil.resetWifiDevice()        --add by kevin.feng@siflower.com.cn 2015/11/10 to reset all online state to 0 and ip to 0.0.0.0 in /etc/config/devlist
...
--add by kevin 2015/09/29 to make the "disabled" consistent with "radio" and send changes to local server(ssst)
        local device_name = dev:name()
        local pure_config = "wifi_info"
        --local sysutil = require "luci.siwifi.sf_sysutil"
        local json = require("luci.json")
        dev:set("radio", shutdown and "0" or "1")
        _uci_real:foreach(pure_config, "band",
            function(s)
                if s.device == device_name then
                    _uci_real:set(pure_config, s[".name"], "enable", shutdown and "0" or "1")
                end
            end
        )
        local changes = _uci_real:changes(pure_config)
        ...
        _uci_real:save(pure_config)
        _uci_real:commit(pure_config)

--add end
		netmd:commit("wireless")
        --sysutil.resetWifiDevice()        --add by kevin.feng@siflower.com.cn 2015/11/10 to reset all online state to 0 and ip to 0.0.0.0 in /etc/config/devlist
		luci.sys.call("env -i /bin/ubus call network reload >/dev/null 2>/dev/null")
		luci.sys.call("env -i /sbin/wifi reload_legacy >/dev/null 2>/dev/null")
