...
function m.parse(map)
	...
	--we have to make sure that we should always in network bridge with ap mode
	if wnet:get("mode") == "ap" and wnet:get("network") ~= "lan" then
	        nixio.syslog("crit","force ap as lan network");
        	wnet:set("network", "lan")
    end 

	if wnet:get("disabled") == "1" then
		luci.http.redirect(luci.dispatcher.build_url("admin/network/wireless"))
		return
	end
	Map.parse(map)
...
local found_sta = nil
local _, net
if wnet:mode() ~= "wds-ap" then
	for _, net in ipairs(wdev:get_wifinets()) do
		if net:mode() == "wds-sta" and net:get("disabled") ~= "1" then
...
if found_sta then
	...
	function ch.formvalue(self, section)
		return {
			...
			m:formvalue(self:cbid(section) .. ".ht_coex") or ""
		}
	end
...
if hwtype == "mac80211" then
	if #tx_power_list > 1 and wnet:mode() ~= "wds-sta" then
	...
	if eal and #eal > 0 and wnet:mode() ~= "wds-sta" then
...
if wnet:mode() == "wds-sta" then
	mode:value("wds-sta", "%s (%s)" % {translate("WDS"), translate("STA")})
else
	mode:value("ap", translate("Access Point"))
	mode:value("sta", translate("Client"))
	mode:value("adhoc", translate("Ad-Hoc"))
	if wnet:mode() == "wds-ap" then
		mode:value("wds-ap", "%s (%s)" % {translate("WDS"), translate("AP")})
	end
end
...
--/* start modified by ivan 2015.8.1 remove "Network" under "Network - Wifi - Interface Configuration - General Setup" */
--[[
...
--]]
--/* end modified by ivan 2015.8.1 remove "Network" under "Network - Wifi - Interface Configuration - General Setup" */


-------------------- MAC80211 Interface ----------------------

if hwtype == "mac80211" then
	if fs.access("/usr/sbin/iw") and wnet:mode() ~= "wds-sta" then
	...
	local wconf = {
		device  = wdev:name(),
		ifname  = wnet:ifname(wnet)..".sta_wds",
		network = "lan",
		mode    = "wds-sta",
		wds     = "1"
	}
	...
	function mode.write(self, section, value)
		local wdsnets = {}
		if value == "sta-wds" then
			if wnet:mode() ~= "wds-ap" then
				m.uci:set("wireless", section, "mode", "wds-ap")
				m.uci:set("dhcp", "lan", "ignore", "1")
				m.uci:commit("dhcp")
				wdev:add_wifinet(wconf)
			end
		else
			if wnet:mode() == "wds-ap" then
				m.uci:set("dhcp", "lan", "ignore", "0")
				m.uci:commit("dhcp")
				wdsnets = wdev:get_wifinets()
				wdev:del_wifinet(wdsnets[2])
			end
...
encr:depends({mode="wds-ap"})
encr:depends({mode="wds-sta"})
...
encr:value("wep-open",   translate("WEP Open System"), {mode="ap"}, {mode="sta"}, {mode="ap-wds"}, {mode="sta-wds"}, {mode="adhoc"}, {mode="ahdemo"}, {mode="wds"}, {mode="wds-ap"}, {mode="wds-sta"})
encr:value("wep-shared", translate("WEP Shared Key"),  {mode="ap"}, {mode="sta"}, {mode="ap-wds"}, {mode="sta-wds"}, {mode="adhoc"}, {mode="ahdemo"}, {mode="wds"}, {mode="wds-ap"}, {mode="wds-sta"})
...
if hwtype == "atheros" or hwtype == "mac80211" or hwtype == "prism2" then
	...
	if hostapd and supplicant then
		encr:value("psk", "WPA-PSK", {mode="ap"}, {mode="sta"}, {mode="ap-wds"}, {mode="sta-wds"}, {mode="wds-ap"}, {mode="wds-sta"})
		encr:value("psk2", "WPA2-PSK", {mode="ap"}, {mode="sta"}, {mode="ap-wds"}, {mode="sta-wds"}, {mode="wds-ap"}, {mode="wds-sta"})
		encr:value("psk-mixed", "WPA-PSK/WPA2-PSK Mixed Mode", {mode="ap"}, {mode="sta"}, {mode="ap-wds"}, {mode="sta-wds"}, {mode="wds-ap"}, {mode="wds-sta"})
		if has_ap_eap and has_sta_eap then
			encr:value("wpa", "WPA-EAP", {mode="ap"}, {mode="sta"}, {mode="ap-wds"}, {mode="sta-wds"}, {mode="wds-ap"}, {mode="wds-sta"})
			encr:value("wpa2", "WPA2-EAP", {mode="ap"}, {mode="sta"}, {mode="ap-wds"}, {mode="sta-wds"}, {mode="wds-ap"}, {mode="wds-sta"})
		end
