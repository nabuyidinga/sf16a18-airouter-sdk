...
--	/* start modified by ivan 2015.7.31 remove "Software", "Startup", "Scheduled Tasks" and "Mount Points" under "System" */
--[[
...
--]]
--	/* end modified by ivan 2015.7.31 remove "Software", "Startup", "Scheduled Tasks" and "Mount Points" under "System" */
...
function action_flashops()
	...
	elseif luci.http.formvalue("image") or luci.http.formvalue("step") then
		...
		elseif step == 2 then
            ...
            if(luci.http.formvalue("keep") ~= "1")  then
                --sysutil.unbind()
            end
            luci.template.render("admin_system/applyreboot", {
				title = luci.i18n.translate("Flashing..."),
				msg   = luci.i18n.translate("The system is flashing now.<br /> DO NOT POWER OFF THE DEVICE!<br /> Wait a few minutes before you try to reconnect. It might be necessary to renew the address of your computer to reach the device again, depending on your settings."),
				addr  = (#keep > 0) and "192.168.4.1" or nil
			})
    ...
    elseif luci.http.formvalue("otaupgrade")  or luci.http.formvalue("otastep") then
        --
        ---- Ota upgrade (added for ota upgrade by xiaoxi on 2015.08.14)
        ----
--[[
        local otastep = tonumber(luci.http.formvalue("otastep") or 1)
        if otastep == 1 then
            local remote_info = {}
            ...
            if (not remote_info) then
                luci.template.render("admin_system/flashops",{
                    reset_avail = reset_avail,
                    upgrade_avail = upgrade_avail,
                    otaversion_undownloaded = true
                })
			elseif (remote_info == 1) then
				luci.template.render("admin_system/error404")
			else
                ...
                if (not string.find(romversion, otaversion)) then
                    ...
                    if (ota_checksum == local_checksum) then
                        luci.template.render("admin_system/otaupgrade", {
                            checksum = ota_checksum,
                            storage  = storage_size(),
                            size     = nixio.fs.stat(image_tmp).size,
                            romversion = romversion,
                            otaversion = otaversion,
                            hold     = (not not luci.http.formvalue("hold"))
                        })
                    else
        ...
        elseif otastep == 2 then
            local hold = (luci.http.formvalue("hold") == "1") and "" or "-n"
            if (luci.http.formvalue("hold") ~= "1") then
                sysutil.unbind()
            end
            luci.template.render("admin_system/applyreboot", {
                title = luci.i18n.translate("Flashing..."),
                msg   = luci.i18n.translate("The system is flashing now.<br /> DO NOT POWER OFF THE DEVICE!<br /> Wait a few minutes before you try to reconnect. It might be necessary to renew the address of your computer to reach the device again, depending on your settings."),
                addr  = (#hold > 0) and "192.168.4.1" or nil
            })
            sysutil.sendSystemEvent(sysutil.SYSTEM_EVENT_UPGRADE)
            sysutil.resetAllDevice()
            fork_exec("sleep 5; killall dropbear uhttpd; sleep 1; /sbin/sysupgrade %s %q" %{ hold, image_tmp })
        end
]]--
	...
	elseif reset_avail and luci.http.formvalue("reset") then
        ...
        if fs.access("/tmp/reset_shortest_time") then
        ...
        if os.time() > reset_shortest_time then
           ...
           --sysutil.send_unbind_command(bindret)
           --sysutil.resetAllDevice()
