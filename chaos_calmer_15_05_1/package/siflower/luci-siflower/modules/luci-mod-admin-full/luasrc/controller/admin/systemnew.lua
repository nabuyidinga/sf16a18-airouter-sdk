...
local wds_enable = false;
    ...
    entry({"admin", "systemnew", "smart_audio"}, template("new_siwifi/device_manager/smart_audio") , _("smart audio"), 9);
	...
	entry({"admin", "systemnew", "get_audio_security_mode"}, call("get_audio_security_mode")).leaf = true;
    entry({"admin", "systemnew", "set_audio_security_mode"}, call("set_audio_security_mode")).leaf = true;
    entry({"admin", "systemnew", "get_green_mode_config"}, call("get_green_mode_config")).leaf = true;
    entry({"admin", "systemnew", "set_green_mode_config"}, call("set_green_mode_config")).leaf = true;
    entry({"admin", "systemnew", "get_green_mode_enable"}, call("get_green_mode_enable")).leaf = true;
    entry({"admin", "systemnew", "set_green_mode_enable"}, call("set_green_mode_enable")).leaf = true;
    entry({"admin", "systemnew", "get_green_mode_device"}, call("get_green_mode_device")).leaf = true;
    entry({"admin", "systemnew", "remove_green_mode_device"}, call("remove_green_mode_device")).leaf = true;
end
...
function get_audio_security_mode()
	--[[
	local t = io.popen("ps | grep sf-mm | wc -l")
	local ret_content = t:read("*all")
	local enable  = tonumber(ret_content) == 3 and 1 or 0
	--]]
	local enable = deviceImpl.get_security_mode()
	local result = {
		code = 0,
		msg = "OK",
		enable = enable
...
function set_audio_security_mode()
	...
	--[[
	if enable == 1 then
		luci.util.exec("/etc/init.d/sfmmd start")
	else
		luci.util.exec("/etc/init.d/sfmmd stop")
	end
	--]]
	if pid == 0 then
...
function get_green_mode_device()
	local result = {
		code = 0,
		msg = "OK",
		device =  deviceImpl.get_green_mode_device()
	}
	sysutil.sflog("INFO","date configure changed!")
	luci.http.prepare_content("application/json")
	luci.http.write_json(result)
