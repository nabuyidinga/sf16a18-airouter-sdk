...
function get_security_mode()
	return tonumber(_uci_real:get("basic_setting", "security_mode", "enable") or 0)
end

function set_security_mode(enable)
	if enable == 1 then
		_uci_real:foreach("wireless", "wifi-iface",
		function(s)
			if s.network == "lan" then
				_uci_real:set("wireless", s[".name"], "hidden", 1)
		...
		function(s)
				_uci_real:set("wireless", s[".name"], "sf_mm", 1)
		end
		)
		_uci_real:foreach("sidefault", "setting",
		function(s)
				_uci_real:set("sidefault", s[".name"], "wl_force", 0)
		end
		)
		_uci_real:set("basic_setting", "security_mode", "enable", 1)
	else
		_uci_real:foreach("wireless", "wifi-iface",
		function(s)
			if s.network == "lan" then
				_uci_real:set("wireless", s[".name"], "hidden", 0)
		...
		_uci_real:foreach("wireless", "wifi-device",
		function(s)
			_uci_real:set("wireless", s[".name"], "sf_mm", 0)
		end
		)
		_uci_real:foreach("sidefault", "setting",
		function(s)
				_uci_real:set("sidefault", s[".name"], "wl_force", 1)
		end
		)
		_uci_real:set("basic_setting", "security_mode", "enable", 0)
	end
	_uci_real:save("sidefault")
	_uci_real:commit("sidefault")
	_uci_real:save("wireless")
	_uci_real:commit("wireless")
	_uci_real:save("basic_setting")
	_uci_real:commit("basic_setting")
...
function get_green_mode_enable()
	local enable = tonumber(_uci_real:get("basic_setting","student_mode","enable"))
	if enable == 0 then
		return 0
	else
		return 1
...
function set_green_mode_enable(enable)
	if tonumber(enable) == 0 then
		_uci_real:set("basic_setting","student_mode","enable",0)
	else
		_uci_real:set("basic_setting","student_mode","enable",1)
	end
	_uci_real:save("basic_setting")
	_uci_real:commit("basic_setting")
	return 0
...
function get_green_mode_config()
	local limit_info = {}
	limit_info["start_time"] = _uci_real:get("basic_setting","timelist","start")
	limit_info["end_time"] = _uci_real:get("basic_setting","timelist","end")
	limit_info["limit"] = _uci_real:get("basic_setting","timelist","limitdown")
	return limit_info
end

function set_green_mode_config(start_time, end_time, limit)
	local mac_list = {}
	_uci_real:set("basic_setting","timelist","start",start_time)
	_uci_real:set("basic_setting","timelist","end",end_time)
	_uci_real:set("basic_setting","timelist","limitdown",limit)
	_uci_real:save("basic_setting")
	_uci_real:commit("basic_setting")

	_uci_real:foreach("wldevlist", "device",
	function(s)
		if s.timelist ~= nil and s.timelist ~= 0 then
			mac_list[#mac_list+1] = s.mac
		end
	end
	)

	for i = 1, #mac_list do
		_uci_real:set("wldevlist",mac_list[i],"timelist", start_time.." "..end_time)
		_uci_real:set("wldevlist",mac_list[i],"limitdown",limit)
	end
...
function get_green_mode_device()
	...
	_uci_real:foreach("wldevlist", "device",
	function(s)
		if s.limitdown ~= nil and s.limitdown ~= "-1" then
			if s.hostname ~= nil then
				mac_list[s.mac] = s.hostname
			else
				mac_list[s.mac] = s.mac
			end
		end
	end
	)
...
function remove_green_mode_device(device)
	if device ~= nil and type(device) == "table" then
		for key, value in pairs(device) do
			_uci_real:foreach("wldevlist", "device",
			function(s)
				if s.mac == key then
					_uci_real:set("wldevlist", s[".name"], "limitdown", -1)
				end
			end
			)
		end
