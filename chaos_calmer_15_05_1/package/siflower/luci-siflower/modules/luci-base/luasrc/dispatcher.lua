...
function authenticator.jsonauth(validator, accs, default)
    ...
    local user = "admin"
    --check passwd for local request
    local localAuth = http.getenv("HTTP_AUTHORIZATION")
    local userid_env = http.getcookie("userid");
    if(localAuth == sys.user.getpasswd(user)) then
        return user,nil,userid_env
    end

    --check for local api
	local arg_list, data_len = luci.http.content()
	local arg_list_table = json.decode(arg_list)
	local sessiontoken = arg_list_table["sessiontoken"]
	local userid_local = arg_list_table["userid"]

    --check if passwd is nil
    if(not sys.user.getpasswd(user)) then
        return user,nil,userid_env or userid_local
    end

    if(sessiontoken and userid_local and userid_local ~= "") then
        --todo: sample judgement here to verify sessiontoken and userid
        return user,nil,userid_local
    end

	local pass = arg_list_table["password"]
    local result = {}
    local code = 1011
    if user and pass then
        if validator(user, pass) then
            return user,nil,userid_local
        else
            code = 1005
        end
...
function authenticator.htmlauth(validator, accs, default)
	...
	if context.urltoken.stok then
		...
		if(not sys.user.getpasswd("admin")) then
			--Siwifi solution
			--TODO: add a conditional judgment
			http.redirect(build_url("admin/quicksettings/admin").."?luci_username=admin")
			--http.redirect(build_url("admin/system/admin").."?luci_username=admin")
		else
...
--    nixio.syslog("crit","httpdispatch--"..tostring(pathinfo))

-- will cause update fail
--	local content= request:formvalue(nil)
--	for i, s in pairs(content) do
--		nixio.syslog("crit","httpdispatch key:"..tostring(i).."--value:"..tostring(s))
--	end
...
function dispatch(request)
	...
	if track.sysauth then
		...
		if not util.contains(accs, user) then
			if authen then
				local user, sess ,userid = authen(sys.user.checkpasswd, accs, def)
				local token
				...
				if not user or not util.contains(accs, user) then
					...
					if not sess then
						...
						if sdat == nil then
							luci.util.exec("/etc/init.d/rpcd restart")
							sdat = util.ubus("session", "create", { timeout = tonumber(luci.config.sauth.sessiontime) })
						end
						if sdat then
					...
					if sess and token then
						--modify by edward: for we need stok in cookie
...
function createindex()
	...
	if indexcache then
		...
		local size = fs.stat(indexcache, "size")
		if cachedate and size and size > 0 then
			local realdate = 0
...
local function _call(self, ...)
    ...
    local userid = getfenv(self.target).userid
    local oldenv = getfenv(func)
    oldenv.userid = userid
	setfenv(func, oldenv)
