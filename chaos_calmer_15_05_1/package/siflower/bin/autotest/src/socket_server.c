...
MSG_POLL:
	...
	if (!strncmp(recvbuf, "active", 6)){
		...
		memcpy(args[arg_index], recvbuf + index, i - index);
		sprintf(str, "scp -i /router/id_rsa %s test@192.168.1.18:~/log/%s", args[1], args[2]);
		sprintf(sendbuf, "get_log success\n");
	}
	else if (!strncmp(recvbuf, "restart", 7)){
		sprintf(str, "killall autotest && /router/autotest &");
		...
		//execute test shell
		sprintf(str, "sh /router/adapter.sh /router/%s", recvbuf);
		system(str);
