...
static void usage()
{
	...
	printf(" ate_cmd wlan1 fastconfig -l 4000 -f 5500 -c 5530 -w 3 -u 3 -m 4 -i 9 -g 0 -p 28 -b 4096 -y, Transmit frames by macbypass on HB\n");
...
int32_t main(int32_t argc, char *argv[])
{
		...
		while ((opt = getopt (argc, argv, options)) != -1)
		{
			switch (opt)
			{
				...
				case 'g':
					...
					break;
				case 'p':
					if(set_phy_power(optarg,channel,bw,mode,rate))
						return -1;
					break;
				case 'v':
	...
	if(argc > 5 && strcmp(argv[0],"ate_cmd") == 0 && strcmp(argv[4],"=") == 0)
	{
		...
		if(strcmp(argv[3],"ATETXPOW") == 0){
			if(set_phy_power(tmpdata,channel,bw,mode,rate))
				return -1;
		}
